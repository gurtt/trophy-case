...
struct Marquee {
	...
	init(_ text: String, inverted: Bool = false) {
		...
		scrollTimer = AnimationController(
			startValue: 0, endValue: Float(-(textWidth + Marquee.padding)), duration: duration)
	}
	...
	static func draw(_ text: String, in bounds: Rect, offset: Float = 0, inverted: Bool = false) {
		...
		defer {
			...
			Graphics.drawBitmap(
				Marquee.scrimImage, at: bounds.origin.translatedBy(dx: bounds.width - 5, dy: 0),
				flip: .flippedX)
		}
		...
		Graphics.drawMode = inverted ? .fillWhite : .copy
		Graphics.drawText(
			text, at: bounds.origin.translatedBy(dx: offset + Float(textWidth + Marquee.padding), dy: 0))
	}
